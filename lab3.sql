...
SHOW INDEX FROM productMarket.order;
ALTER TABLE ProductMarket.order
add CONSTRAINT product_connect FOREIGN KEY (product_ID)
REFERENCES productMarket.product (ID) ON DELETE NO ACTION ON
UPDATE NO ACTION;
...
ALTER TABLE productMarket.order
add test2 int;
SHOW CREATE TABLE productMarket.ORDER;
...
SHOW CREATE TABLE productMarket.USER;
ALTER TABLE productMarket.user
MODIFY COLUMN FIRSTNAME CHAR(60) NULL;
SHOW CREATE TABLE productMarket.USER;

SHOW CREATE TABLE productMarket.product_inventory;
ALTER TABLE productMarket.PRODUCT_inventory
...
(quantity, versionID);
DESCRIBE productMarket.product_inventory;

SHOW INDEX FROM productMarket.order;
ALTER TABLE ProductMarket.order
add CONSTRAINT product_connect FOREIGN KEY (product_ID)
REFERENCES productMarket.product_inventory (quantity) ON DELETE CASCADE;
SHOW INDEX FROM productMarket.order;